...
import type { IEnrollmentRepository } from '../models/student.model.js';
...
export class EnrollmentRepository implements IEnrollmentRepository {
  async insertEnrollment(enrollment: Enrollment): Promise<Enrollment> {
    const query = `
  ...
  async selectEnrollmentsByStudent(studentId: string): Promise<EnrollmentDetails[]> {
    const query = `
  ...
  async selectAllEnrollments(shop: string): Promise<EnrollmentDetails[]> {
    const query = `
      SELECT e.*, s.studentName, s.email as studentEmail, c.courseTitle, c.category, c.duration
      FROM enrollments e
      JOIN students s ON e.studentId = s.id
  ...
  async selectRecentEnrollments(shop: string, limit = 5): Promise<EnrollmentDetails[]> {
    const query = `
      SELECT e.*, s.studentName, s.email as studentEmail, c.courseTitle, c.category, c.duration
      FROM enrollments e
      JOIN students s ON e.studentId = s.id
  ...
  async updateEnrollmentStatusInDb(
    id: string,
  ...
  async deleteEnrollmentFromDb(id: string, shop: string): Promise<boolean> {
    const query = `
  ...
  async getMerchantDashboardMetrics(shop: string): Promise<{
    totalCourses: number;
